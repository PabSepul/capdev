...
assert.match(second.document.querySelector('#continue-description').textContent, /proyecto 2 de 50/);
assert.equal(second.document.querySelector('[data-learning-route="python"] .route-progress').hidden, false);
assert.match(second.document.querySelector('[data-learning-route="python"] [data-route-progress]').textContent, /1\/50/);
second.close();
...
third.LearningState.definirPlanSemanal('steady');
...
assert.equal(third.document.querySelectorAll('#weekly-sessions li').length, 3);
assert.equal(third.document.querySelectorAll('#weekly-sessions .is-complete').length, 3);
third.document.querySelector('#weekly-toggle-pause').click();
assert.equal(third.LearningState.metaSemanal().pausada, true);
assert.match(third.document.querySelector('#weekly-home-copy').textContent, /en pausa/);
third.close();
