...
begin
  ...
  for op in select value from jsonb_array_elements(p_operations) loop
    ...
    n := case when rid in ('python','html-css','javascript','sql','git','apis','terminal','regex','ia','datos-python','nodejs','typescript','react','json','markdown','accesibilidad','testing','docker','mongodb') then 50 else 0 end;
