...
alter table public.learning_profiles add column if not exists weekly_pace text;
alter table public.learning_profiles add column if not exists weekly_target smallint;
alter table public.learning_profiles add column if not exists weekly_started_on date;
alter table public.learning_profiles add column if not exists weekly_baseline integer;
...
alter table public.learning_profiles drop constraint if exists learning_profiles_weekly_pace_check;
alter table public.learning_profiles add constraint learning_profiles_weekly_pace_check check (weekly_pace is null or weekly_pace in ('light','steady','focused'));
alter table public.learning_profiles drop constraint if exists learning_profiles_weekly_target_check;
alter table public.learning_profiles add constraint learning_profiles_weekly_target_check check (weekly_target is null or weekly_target between 1 and 20);
alter table public.learning_profiles drop constraint if exists learning_profiles_weekly_baseline_check;
alter table public.learning_profiles add constraint learning_profiles_weekly_baseline_check check (weekly_baseline is null or weekly_baseline between 0 and 100000);
...
declare
  ...
  selected_pace text; selected_target integer; selected_week date; selected_baseline integer;
  ...
  for op in select value from jsonb_array_elements(p_operations) loop
    ...
    if k = 'weekly-plan' then
      ...
      update public.learning_profiles set weekly_pace=selected_pace, weekly_target=selected_target,
        weekly_started_on=selected_week, weekly_baseline=selected_baseline where user_id=uid;
