...
for (const file of ["starter-exams.js", "api-lab.js", "apis-course.js", "course-expansion.js", "apis-fifty-course.js"])
  vm.runInContext(read(file), sandbox, { filename: file });

const course = sandbox.ApisCourse;
...
assert.equal(sandbox.StarterExams.LEVEL_EXAMS.apis.length, 13);
...
for (const item of modules) {
  ...
  const example = sandbox.ApiLab.run(item.example);
  assert.equal(example.error, null, `módulo ${item.id}: ejemplo incompatible: ${example.error}`);
  assert.ok(example.respuestas.length > 0, `módulo ${item.id}: el ejemplo produce respuestas`);
  const starter = sandbox.ApiLab.run(item.starter);
  ...
  const solution = sandbox.ApiLab.run(item.solution);
...
for (const exam of sandbox.StarterExams.LEVEL_EXAMS.apis.slice(4)) {
...
console.log(`APIs 17–50: 9 niveles, 34 módulos, ${validations} validaciones y 9 exámenes: OK`);
