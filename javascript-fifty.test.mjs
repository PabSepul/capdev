...
for (const file of [
  "starter-runtime.js", "base-courses.js", "starter-exams.js", "learning-guidance.js",
  "course-expansion.js", "javascript-learning.js", "javascript-fifty-course.js"
]) vm.runInContext(read(file), sandbox, { filename: file });

const course = sandbox.JavaScriptCourse;
assert.equal(course.levels.length, 13, "JavaScript debe tener trece niveles");
...
assert.equal(sandbox.StarterExams.LEVEL_EXAMS.javascript.length, 13);
...
const nativeText = code => {
  const output = [];
  vm.runInNewContext(code, { console: { log: (...values) => output.push(values.map(String).join(" ")) } });
  return output.join("\n");
};
...
for (const item of modules) {
  ...
  const example = sandbox.StarterRuntime.runJavaScript(item.example);
  assert.equal(example.error, null, `módulo ${item.id}: ejemplo incompatible: ${example.error}`);
  assert.equal(example.text, nativeText(item.example), `módulo ${item.id}: el ejemplo difiere de JavaScript real`);

  const starter = sandbox.StarterRuntime.runJavaScript(item.starter);
  assert.equal(starter.error, null, `módulo ${item.id}: código inicial incompatible: ${starter.error}`);
  assert.equal(item.checks.every(entry => entry.test(item.starter, starter)), false, `módulo ${item.id}: el inicio exige intervenir`);

  const solution = sandbox.StarterRuntime.runJavaScript(item.solution);
  assert.equal(solution.error, null, `módulo ${item.id}: solución incompatible: ${solution.error}`);
  assert.equal(solution.text, nativeText(item.solution), `módulo ${item.id}: la solución difiere de JavaScript real`);
...
for (const exam of sandbox.StarterExams.LEVEL_EXAMS.javascript.slice(4)) {
...
console.log(`JavaScript 17–50: 9 niveles, 34 módulos, ${validations} validaciones y 9 exámenes: OK`);
