...
for (const file of [
  "starter-runtime.js", "sql-course.js", "starter-exams.js", "learning-guidance.js",
  "course-expansion.js", "sql-fifty-course.js"
...
assert.equal(sandbox.StarterExams.LEVEL_EXAMS.sql.length, 13);
...
for (const item of modules) {
  ...
  const example = sandbox.StarterRuntime.runSql(item.example);
  assert.equal(example.error, null, `módulo ${item.id}: ejemplo incompatible: ${example.error}`);
  assert.ok(example.rows.length > 0, `módulo ${item.id}: el ejemplo muestra un resultado`);
  const starter = sandbox.StarterRuntime.runSql(item.starter);
  assert.equal(starter.error, null, `módulo ${item.id}: inicio incompatible`);
  assert.equal(item.checks.every(entry => entry.test(item.starter, starter)), false, `módulo ${item.id}: el inicio exige intervenir`);
  const solution = sandbox.StarterRuntime.runSql(item.solution);
  assert.equal(solution.error, null, `módulo ${item.id}: solución incompatible: ${solution.error}`);
  assert.equal(solution.text, item.expected.text, `módulo ${item.id}: la referencia corresponde a la solución`);
...
for (const exam of sandbox.StarterExams.LEVEL_EXAMS.sql.slice(4)) {
...
console.log(`SQL 17–50: 9 niveles, 34 módulos, ${validations} validaciones y 9 exámenes: OK`);
